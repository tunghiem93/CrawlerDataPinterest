...
namespace CMS_Shared.CMSGroupBoard
{
    public class CMSGroupBoardFactory
    {
        private static Semaphore m_Semaphore = new Semaphore(1, 1); /* semaphore for create key, craw data */

        public List<CMS_GroupBoardsModels> GetList()
        {
            try
            {
                ...
                {
                    /* get all key word */
                    var data = _db.CMS_GroupBoard.Where(o => o.Status == (byte)Commons.EStatus.Active).Select(o => new CMS_GroupBoardsModels()
                    {
                        Id = o.ID,
                        Sequence = o.Sequence,
                        Name = o.Name,
                    ...
                    /* update quantity */
                    var listCount = _db.CMS_R_GroupBoard_Board.Where(o => o.Status != (byte)Commons.EStatus.Deleted)
                        .Join(_db.CMS_R_Board_Pin.Where(o=> o.Status != (byte)Commons.EStatus.Deleted), g=> g.BoardID, k=>k.BoardID, (g,k)=> new { g, k })
                        .GroupBy(o => o.g.GroupBoardID)
                        .Select(o => new
                        {
                            Id = o.Key,
                            Quantity = o.Count(),
        ...
        public CMS_GroupBoardsModels GetDetail(string id)
        {
            try
            {
                using (var _db = new CMS_Context())
                {
                    /* get all key word */
                    var data = _db.CMS_GroupBoard.Where(o => o.Status == (byte)Commons.EStatus.Active && o.ID == id).Select(o => new CMS_GroupBoardsModels()
                    {
                        Id = o.ID,
                        Sequence = o.Sequence,
                        Name = o.Name,
                    ...
                    /* update quantity */
                    CMSBoardFactory groupFac = new CMSBoardFactory();
                    data.ListBoardOnGroup = groupFac.GetList(id);
        ...
        public bool Delete(string Id, string createdBy, ref string msg)
        {
            ...
            try
            {
                using (var _db = new CMS_Context())
                {
                    var group = _db.CMS_GroupBoard.Where(o => o.ID == Id).FirstOrDefault();
                    ...
                    if (group != null)
                    {
                        ...
                        /* loop list group key */
                        var listGroupKey = _db.CMS_R_GroupBoard_Board.Where(o => o.GroupBoardID == group.ID).ToList();
