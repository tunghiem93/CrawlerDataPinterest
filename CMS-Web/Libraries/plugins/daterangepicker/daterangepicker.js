...
(function() {
  ...
  $.fn.daterangepickerknockout = function(options, callback) {
    if (options == null) {
      options = {};
    }
    this.each(function() {
      var $element;
      $element = $(this);
      if (!$element.data('daterangepickerknockout')) {
        options.anchorElement = $element;
        if (callback) {
          options.callback = callback;
        }
        options.callback = $.proxy(options.callback, this);
        return $element.data('daterangepickerknockout', new DateRangePickerView(options));
      }
    });
    return this;
  };
  ...
  ko.bindingHandlers.daterangepickerknockout = (function() {
    return $.extend(this, {
      ...
      init: function(element, valueAccessor, allBindings) {
        ...
        return $(element).daterangepickerknockout(options, function(startDate, endDate, period) {
      ...
      update: function(element, valueAccessor, allBindings) {
        ...
        return ko.ignoreDependencies(function() {
          var text;
          if (!$element.data('daterangepickerknockout').standalone()) {
            text = $element.data('daterangepickerknockout').single() ? startDateText : startDateText + " – " + endDateText;
            $element.val(text).text(text);
          }
          $element.data('daterangepickerknockout').startDate(startDate);
          return $element.data('daterangepickerknockout').endDate(endDate);
        });
      }
    });
  })();
  ...
  DateRangePickerView = (function() {
    function DateRangePickerView(options) {
      ...
      if (this.anchorElement) {
        ...
        if (!this.standalone()) {
          $(document).on('mousedown.daterangepickerknockout', this.outsideClick).on('touchend.daterangepickerknockout', this.outsideClick).on('click.daterangepickerknockout', '[data-toggle=dropdown]', this.outsideClick).on('focusin.daterangepickerknockout', this.outsideClick);
        }
      }
      if (this.opened()) {
        this.updatePosition();
